...
//   }
//
// // internalDoFilter的最终目的也只有一个，就是调当前pos指向的过滤器链中的某一个filter的doFilter(request, response, this)方法，
// // 中间可能会增加一些安全策略，以及当所有过滤器调用完了，进行的一些收尾清理工作，包括调用servlet.service(request, response)方法，来处理真正的请求，
// // 以及清除threadLocal中保存的当前的request和response，为下一次请求做准备。
// //
...
//             .equalsIgnoreCase(e1.getFilterDef().getAsyncSupported())) {
...
//           SecurityUtil
//               .doAsPrivilege("service", this.servlet, classTypeUsedInService, args, principal);
//         } else {
